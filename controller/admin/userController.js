...
const addUser = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(User,[ 'email' ],dataToCreate,'INSERT');
...
const bulkInsertUser = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(User,[ 'email' ],dataToCreate,'BULK_INSERT');
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const updateUser = async (req,res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(User,[ 'email' ],dataToUpdate,'UPDATE', query);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const bulkUpdateUser = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(User,[ 'email' ],dataToUpdate,'BULK_UPDATE', filter);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
