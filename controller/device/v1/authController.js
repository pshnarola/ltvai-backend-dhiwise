...
const register = async (req,res) =>{
  try {
    ...
    let checkUniqueFields = await common.checkUniqueFieldsInDatabase(User,[ 'email', 'username' ],data,'REGISTER');
...
const validateResetPasswordOtp = async (req,res) =>{
  ...
  try {
    ...
    let found = await dbService.findOne(User, { 'resetPasswordLink.code': params.otp });
...
const resetPassword = async (req,res) => {
  ...
  try {
    ...
    let found = await dbService.findOne(User, { 'resetPasswordLink.code': params.code });
    if (!found || !found.resetPasswordLink.expireTime) {
