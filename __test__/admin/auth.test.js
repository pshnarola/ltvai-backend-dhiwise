...
beforeAll(async function (){
  try {
    ...
    const city = dbInstance.collection('cities');
    insertedCity = await city.insertOne({
      name: 'Shari Howe',
      stateId: 'Practical',
      cityId: 'Soft',
      id: '6299b84d74d63acbc5217349'
    });
    const state = dbInstance.collection('states');
    insertedState = await state.insertOne({
      name: 'Ms. Eduardo Trantow',
      countryId: 'global',
      country_code: 'capacitor',
      country_name: 'Concrete',
      state_code: 'Hawaii',
      stateId: 'Knolls',
      id: '6299b84d74d63acbc521734a'
    });
    const country = dbInstance.collection('countries');
    insertedCountry = await country.insertOne({
      name: 'Clara Maggio',
      code: 'Towels',
      iso3: 'transmitting',
      countryId: 'Money',
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Maudie.Herman',
        'email':'Jamir.Kirlin@hotmail.com',
        'password':'cZaniilsrJH8pRQ',
        'clientId':'Soap',
        'userType':authConstant.USER_TYPES.Admin,
        'mobileNo':'(249) 658-4037',
        'forgotPasswordToken':'Soap',
        'emailVerified':false,
        'firstName':'Eduardo Cronin',
        'lastName':'Julian Terry',
        'telephone':'Customer',
        'telephoneCountryCode':'Incredible',
        'address1':'infrastructures',
        'address2':'Manager',
        'country':insertedCountry.insertedId,
        'state':insertedState.insertedId,
        'city':insertedCity.insertedId,
        'isEnableTwoFactorAuth':true,
        'postalCode':'Licensed',
        'companyName':'Keyboard',
        'weeklyMetrics':true,
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Jamir.Kirlin@hotmail.com', });
