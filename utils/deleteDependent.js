...
let CustomerAccounts = require('../model/customerAccounts');
...
const deleteCity = async (filter) =>{
  try {
    ...
    if (city && city.length){
      ...
      const userFilter = { $or: [{ city : { $in : city } }] };
...
const deleteState = async (filter) =>{
  try {
    ...
    if (state && state.length){
      ...
      const userFilter = { $or: [{ state : { $in : state } }] };
      const userCnt = await dbService.deleteMany(User,userFilter);
...
const deleteCountry = async (filter) =>{
  try {
    ...
    if (country && country.length){
      ...
      const userFilter = { $or: [{ country : { $in : country } }] };
      const userCnt = await dbService.deleteMany(User,userFilter);
...
const deleteCustomerAccounts = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(CustomerAccounts,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const customerAccountsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const customerAccountsCnt = await dbService.deleteMany(CustomerAccounts,customerAccountsFilter);
      ...
      let response = {
        ...
        state :stateCnt,
        country :countryCnt,
        customerAccounts :customerAccountsCnt,
...
const countCustomerAccounts = async (filter) =>{
  try {
    const customerAccountsCnt =  await dbService.count(CustomerAccounts,filter);
    return { customerAccounts : customerAccountsCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const customerAccountsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const customerAccountsCnt =  await dbService.count(CustomerAccounts,customerAccountsFilter);
...
const softDeleteCustomerAccounts = async (filter,updateBody) =>{  
  try {
    const customerAccountsCnt =  await dbService.updateMany(CustomerAccounts,filter);
    return { customerAccounts : customerAccountsCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const customerAccountsFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const customerAccountsCnt = await dbService.updateMany(CustomerAccounts,customerAccountsFilter,updateBody);
